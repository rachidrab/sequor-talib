...
import com.pfe.sequortalib.config.KafkaProperties;
...
class SequorTalibKafkaResourceIT {
    ...
    void setup() {
        ...
        SequorTalibKafkaResource kafkaResource = new SequorTalibKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/sequor-talib-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/sequor-talib-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
